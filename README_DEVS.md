...
- `code/webpage_utils.py`
  - Shared HTTP fetching, HTML extraction, and text utilities used by all four web skill modules (WebExtract, WebMine, PageAssess, WebSearch).
...
3. Load `code/skills/skills_summary.md` and ask the planner LLM which skills to call (returns JSON).
4. Execute the approved Python skill calls in order and collect outputs.
5. Build the final enriched prompt from skill outputs, recalled memories, and the planner template.
6. Call the final LLM and validate the response.
7. Retry up to `MAX_ITERATIONS` times if validation fails, feeding back error context.
8. Log everything - planner prompt, plan JSON, skill outputs, final prompt, response, validation, and TPS for each LLM phase.
...
**Pass 1 - `orchestration.py`** (before the planner sees the prompt):
```python
user_prompt = resolve_tokens(user_prompt)
```
A scheduled prompt like `"web mine UK news for {today}"` arrives at the planner as `"web mine UK news for 2026-03-11"`. This is the primary pass for all user-facing and scheduled prompts.
...
## Web Skills: WebSearch vs WebExtract vs WebMine vs PageAssess
...
| | WebSearch | WebExtract | WebMine | PageAssess |
|---|---|---|---|---|
...
- Does *not* save results to disk; if persistence is required, use WebMine (`mine_search`) instead.
...
- Does *not* save content to disk; if persistence is required, use WebMine (`mine_url`) instead.
...
3. **User wants to save / archive / mine / file content for later → WebMine**
4. **User says "mine", "save to the X research area", "file under X domain", "store in X" → always WebMine**, never WebSearch or WebExtract
5. **User gives a URL and explicitly wants it saved → WebMine (`mine_url`)**
6. **User gives a query and explicitly wants results saved → WebMine (`mine_search`)**
...
- **WebMine is persistent** - it always writes a file to `webresearch/01-Mine/`.
