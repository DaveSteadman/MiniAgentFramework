...
1. **Search** - call `koredata_search(query, domains=[...], limit=5)`.  Omit `domains` to
   search all three services at once.  The response contains a `results` dict keyed by domain
   (`"feeds"`, `"reference"`, `"library"`), each holding a list of summary dicts.
...
- `domains` *(optional)* - list of services to search: `"feeds"`, `"reference"`, `"library"`.
  Omit or pass `None` to search all three.
...
{
  "query": "...",
  "domains_searched": ["feeds", "reference", "library"],
  "results": {
...
- Unknown or cross-cutting - omit `domains` to search all three at once
